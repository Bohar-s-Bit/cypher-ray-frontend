...
const LoginForm = ({ onSubmit, loading = false }) => {
  ...
  const handleAccessRequestSubmit = async (e) => {
    ...
    const formData = new FormData(e.target);
    const data = {
      fullName: formData.get("accessName"),
      email: formData.get("accessEmail"),
      organizationName: formData.get("accessOrganization"),
      reasonForJoining: formData.get("accessMessage"),
    };

    setAccessLoading(true);
    try {
      const response = await authService.requestAccess(data);
  ...
  return (
    <div className="relative bg-white rounded-[30px] shadow-[0_10px_30px_rgba(0,0,0,0.3)] overflow-hidden w-[1024px] max-w-[95vw] min-h-[600px]">
      ...
      >
        ...
        >
          <h1 className="text-neutral-900 text-2xl md:text-3xl font-bold mb-4 md:mb-6">
            Request Access
          </h1>
          <span className="text-xs md:text-sm text-neutral-600 mb-4 md:mb-6 text-center">
      ...
      >
        ...
        >
          ...
          <span className="text-xs md:text-sm text-neutral-600 mb-4 md:mb-6">
          ...
          <div className="flex items-center justify-between w-full mt-4 md:mt-5 mb-3 md:mb-4">
            <div className="flex items-center gap-2">
              ...
              <label
                htmlFor="rememberMe"
                className="text-neutral-700 text-sm md:text-base cursor-pointer"
              >
                Remember me
              </label>
            </div>
